...
it('parses github user', async () => {
  ...
  expect(data.service).toEqual('github')
  expect(data.id).toEqual(ghUser.id)
  expect(data.name).toEqual(ghUser.login)
  expect(data.email).toEqual(ghUserEmails[0].email)
  expect(data.picture).toEqual(ghUser.avatar_url)
})
